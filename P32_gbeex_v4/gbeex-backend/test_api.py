...
def main():
    ...
    assert_and_log(access_token is not None, "Access token received.", "No access_token in login response.", exit_on_fail=True)
    assert_and_log(refresh_token is not None, "Refresh token received.", "No refresh_token in login response.", exit_on_fail=True)
    ...
    r = retry_request("GET", ME_URL, headers=auth_headers(access_token))
    ...
    logger.info(f"Attempting to refresh token using refresh_token: {refresh_token[:10]}...") 
    
    # Introduce a tiny delay to ensure timestamps in new tokens are different
    time.sleep(0.01) 

    r = retry_request("POST", REFRESH_URL, headers=auth_headers(refresh_token))
    assert_and_log(r.status_code == 200, "Token refresh successful (200 OK).", f"Token refresh failed: Status {r.status_code}, Response: {r.text}")
    
    refreshed_data = r.json()
    new_access_token = refreshed_data.get("access_token")
    new_refresh_token = refreshed_data.get("refresh_token")

    assert_and_log(new_access_token is not None, "New access token received from refresh.", "No new access_token in refresh response.", exit_on_fail=True)
    assert_and_log(new_refresh_token is not None, "New refresh token received from refresh.", "No new refresh_token in refresh response.", exit_on_fail=True)
    
    # Asserting that the new tokens are different. This relies on `time.sleep`
    # and the granularity of `datetime.now(timezone.utc)` for distinct timestamps.
    # The crucial part is that they are *valid* new tokens, which is checked next.
    assert_and_log(new_access_token != access_token, "New access token is structurally different from old.", "New access token is identical to old, should be different.")
    assert_and_log(new_refresh_token != refresh_token, "New refresh token is structurally different from old.", "New refresh token is identical to old, should be different.")

    # Use the newly obtained access token to verify it's valid
    logger.info("Verifying new access token by calling /users/me...")
    r_new_token = retry_request("GET", ME_URL, headers=auth_headers(new_access_token))
    assert_and_log(r_new_token.status_code == 200, "/users/me with new token returned 200 OK.", f"/users/me with new token failed: Status {r_new_token.status_code}, Response: {r_new_token.text}")
    assert_and_log(r_new_token.json().get("username") == USERNAME, "User data with new token matches expected username.", "Username mismatch with new token.")
    
    logger.info("Token Refresh test completed.")

    # Remaining tests (using the original access_token, assuming its validity for these short tests)

    # 6) companies list & detail
    logger.info("\n=== TEST: Companies Endpoints ===")
    r = retry_request("GET", COMPANIES_URL, headers=auth_headers(access_token)) 
    ...
    r = retry_request("GET", f"{COMPANIES_URL}/{cid}", headers=auth_headers(access_token))
    ...
    r = retry_request("GET", f"{COMPANIES_URL}/{cid}/protocols", headers=auth_headers(access_token))
    ...
    r = retry_request("GET", f"{BASE_URL}/api/v1/protocols/{pid}/sites", headers=auth_headers(access_token))
    ...
    # 8) notifications & mark-read
    logger.info("\n=== TEST: Notifications Endpoints ===")
    r = retry_request("GET", NOTIFS_URL, headers=auth_headers(access_token))
    ...
    if notifs:
        ...
        r = retry_request("POST", f"{NOTIFS_URL}/{nid}/read", headers=auth_headers(access_token))
        ...
        updated_notifs = retry_request("GET", NOTIFS_URL, headers=auth_headers(access_token)).json()
    ...
    r = retry_request("GET", PORTLETS_URL, headers=auth_headers(access_token))
    ...
    r = retry_request("POST", PORTLETS_URL, headers=auth_headers(access_token), json=new_portlet_payload)
